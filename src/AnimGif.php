...
class AnimGif
{
    ...
    public function create($frames, $durations = self::DEFAULT_DURATION, $loop = 0)
    {
        ...
        foreach ($frames as $frame) {
            ...
            for ($j = (13 + 3 * (2 << (ord($this->frameSources[$i]{
            10}) & 0x07))), $k = TRUE; $k; $j++) {

                switch ($this->frameSources[$i]{
                $j}) {
    ...
    protected function gifAddHeader()
    {
        ...
        if (ord($this->frameSources[0]{
        10}) & 0x80) {

            $cmap = 3 * (2 << (ord($this->frameSources[0]{
            10}) & 0x07));
    ...
    {
        $Locals_str = 13 + 3 * (2 << (ord($this->frameSources[$i]{
        10}) & 0x07));

        $Locals_end = strlen($this->frameSources[$i]) - $Locals_str - 1;
        $Locals_tmp = substr($this->frameSources[$i], $Locals_str, $Locals_end);

        $Global_len = 2 << (ord($this->frameSources[0]{
        10}) & 0x07);
        $Locals_len = 2 << (ord($this->frameSources[$i]{
        10}) & 0x07);

        $Global_rgb = substr($this->frameSources[0], 13, 3 * (2 << (ord($this->frameSources[0]{
        10}) & 0x07)));
        $Locals_rgb = substr($this->frameSources[$i], 13, 3 * (2 << (ord($this->frameSources[$i]{
        10}) & 0x07)));

        $Locals_ext = "!\xF9\x04" . chr(($this->dis << 2) + 0) . word2bin($d) . "\x0\x0";

        if ($this->transparent_color > -1 && ord($this->frameSources[$i]{
        10}) & 0x80) {

            for ($j = 0; $j < (2 << (ord($this->frameSources[$i]{
            10}) & 0x07)); $j++) {

                if (
                    ord($Locals_rgb{
                    3 * $j + 0}) == (($this->transparent_color >> 16) & 0xFF) &&
                    ord($Locals_rgb{
                    3 * $j + 1}) == (($this->transparent_color >> 8) & 0xFF) &&
                    ord($Locals_rgb{
                    3 * $j + 2}) == (($this->transparent_color >> 0) & 0xFF)
                ) {
        ...
        if (ord($this->frameSources[$i]{
        10}) & 0x80 && $this->imgBuilt) {
